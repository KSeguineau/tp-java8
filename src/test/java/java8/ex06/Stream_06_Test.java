...
public class Stream_06_Test {
	...
	// Soit une méthode impérative qui permet de construire une somme des
	// chiffres de 1 à n
	...
	private long iterateSum(long n) {
		Optional<Long> sum = Stream.iterate(0L, t -> t + 1).limit(n).reduce((a, b) -> a + b);
		return sum.get();
	...
	private long parallelIterateSum(long n) {
		Optional<Long> sum = Stream.iterate(0L, t -> t + 1).limit(n).parallel().reduce((a, b) -> a + b);
	...
	// Voici une méthode qui exécute 10 fois un traitement et retourne le
	// meilleur temps (le plus court)
	...
	private long rangeSum(long n) {
		long l = LongStream.range(0, n).sum();
		return l;
	...
	private long rangeParallelSum(long n) {
		long l = LongStream.range(0, n).parallel().sum();
	...
	// Les performances de traitements en parallèle dépendent de la capacité
	// d'une structure à se décomposer.
	// Stream.iterate() conçu pour générer un flux continue infinie ne se
	// décompose pas alors qu'une structure finie comme
