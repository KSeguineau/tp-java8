...
public class Stream_08_Test {
	...
	Function<List<Pizza>, List<Pizza>> filterPizza = t -> t.stream().filter(a -> a.getPrice() >= 1000).collect(
			Collectors.toList());

	// TODO créer une fonction List<Pizza> -> List<Pizza>
	// TODO seules les pizzas ayant un prix >= 1000 sont conservées
	// TODO .parallel()
	Function<List<Pizza>, List<Pizza>> parallelFilterPizza = t -> t.stream().parallel().filter(a -> a
			.getPrice() >= 1000).collect(Collectors.toList());
